...
# Useful for stats about the performance of the algorithm
...
def run(folder_path, roi_file, history=500, dist2_threshold=400.0, learning_rate=-1):
    ...
    print(f"  history={history}  dist2Threshold={dist2_threshold}  learningRate={learning_rate}")

    # KNN without shadow detection
    ...
    for i in range(start_frame, end_frame + 1):
        ...
        mask = knn.apply(frame, learningRate=learning_rate)

        accumulate_stats(gt_img, mask, stats)

        cv2.imshow("Frame",    frame)
        cv2.imshow("KNN mask", mask)

        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

    print(f"F1 – KNN: {f1_score(stats):.4f}")
